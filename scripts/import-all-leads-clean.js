...
const LOG_PATH = path.join(__dirname, '../import-clean-log.json');
...
function convertLeadToNewFormat(parts, newId, userMap) {
  ...
  const consultantEmail = userMap[consultantName] || '';
...
async function importAllLeads() {
  console.log('🚀 Temiz lead import işlemi başlatılıyor...\n');
  ...
  try {
    ...
    console.log(`✅ ${log.validRecords} geçerli kayıt bulundu\n`);
    ...
    const existingPhones = new Set(
      existingData.map(item => {
        if (typeof item.personal === 'object') {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      try {
        ...
        const consultantName = parts[2].trim();
        ...
        // Kullanıcı kontrolü
        if (consultantName && !userMap[consultantName]) {
          if (!log.missingUsers.includes(consultantName)) {
            log.missingUsers.push(consultantName);
          }
        }
        
        // Yeni formata dönüştür
        const newLead = convertLeadToNewFormat(parts, nextId, userMap);
    ...
    // Özet
    console.log('\n' + '='.repeat(50));
    console.log('✅ TEMİZ İMPORT İŞLEMİ TAMAMLANDI!');
    ...
    if (log.missingUsers.length > 0) {
      console.log(`\n⚠️  Sistemde olmayan kullanıcılar: ${log.missingUsers.length}`);
      log.missingUsers.forEach((user, idx) => {
